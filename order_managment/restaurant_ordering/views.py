...
def cooking_list(request):
    ...
    orders = Order.objects.filter(status_cook=False)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('restaurant_ordering:cooking_list')  # Перенаправляем на тот же список после изменения
...
def delivery_list(request):
    ...
    orders = Order.objects.filter(status_cook=True).exclude(payment_state=True, status_delivery=True)
